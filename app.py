...
app.config['ALLOWED_EXTENSIONS'] = {'xlsx'}  # Define allowed extensions
...
@app.route('/add_garages_from_excel', methods=['GET', 'POST'])
@login_required
def add_garages_from_excel():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            for index, row in data.iterrows():
                garage = Garage(
                    ...
                    contact=row['contact'],
                    phone=row['phone'],
                    last_visit="עדיין לא ביקרתי",
                    brands=row['brands'],
                    garage_num=row['garage_num']
                )
                db.session.add(garage)
            db.session.commit()
            flash('Garages added successfully from Excel file!')
...
def delete(snum):
    ...
    db.session.commit()
    flash(f'Garage {snum} deleted successfully!')
    return redirect(url_for('index'))
...
def visit(snum):
    ...
    db.session.commit()
    flash(f'Garage {snum} marked as visited successfully!')
